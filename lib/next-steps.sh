...
CLEAN="./$RECOVERY_DIR/clean-repo.sh"
{
  ...
  if [[ -s "$RESTORABLE" ]] && prc_evidence_is_volatile "$OUT"; then
    printf '## Read this before you reboot\n\n'
    ...
    printf '%s of these repositories are **restore candidates**. `restore.sh` reads the\n' "$(awk 'END{print NR}' "$RESTORABLE")"
    printf 'pre-attack commit out of the local mirror, and those commits are unreachable\n'
    printf 'on GitHub now, so a restart may be the moment they stop being recoverable.\n\n'
    printf 'Either finish the restore in this session, or copy those mirrors somewhere\n'
    printf 'that survives, and delete them when you are done:\n\n'
    printf '```bash\nmkdir -p ~/polinrider-keep\n'
    printf 'while read -r repo; do cp -R "%s/${repo##*/}.git" ~/polinrider-keep/; done < %s\n' "$OUT" "$RESTORABLE"
    printf '```\n\n'
    printf 'Everything else here is regenerated by re-running the scan.\n\n'
    printf -- '---\n\n'
  fi
...
if [[ -s "$RESTORABLE" ]] && prc_evidence_is_volatile "$OUT"; then
  printf '  Evidence is in a temporary directory and your machine clears it on\n'
  printf '  restart. %s repo(s) are restore candidates whose pre-attack commits now\n' "$(awk 'END{print NR}' "$RESTORABLE")"
  printf '  exist only in those mirrors. Finish the restore this session, or copy\n'
  printf '  them somewhere that survives. See NEXT-STEPS.md.\n\n'
fi
